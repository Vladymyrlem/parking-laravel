...
jQuery(function () {
    ...
    function includes_dates(arr1, arr2) {
        return arr1.filter(date1 => {
            return arr2.some(date2 => {
...
// Create an empty object to store the key-value pairs
        const pricesObj = {};
        const startPromotional = $('.start-promotional-value').text();
        const endPromotional = $('.end-promotional-value').text();

        const arrivalDate = moment(startDate + ' ' + startTime, 'YYYY-MM-DD HH:mm:ss');
        const departureDate = moment(endDate + ' ' + endTime, 'YYYY-MM-DD HH:mm:ss');
...
// Check if the differenceDays is greater than 15
        if (daysDifference > 15) {
            ...
            const promoPriceElement = $('#day-15').find('h4');

            if ($('#day-15').hasClass('promo')) {
                let priceFor15Days = parseFloat(promoPriceElement.find('span.promotional-price-value').text());
                console.log('Promotional 15 days');
                console.log('Promotional 15 Price' + priceFor15Days);
                totalPrice = priceFor15Days + additionalPrice;

            } else {
                let priceFor15Days = pricesObj['15'];
                console.log('Standart 15 Price' + priceFor15Days);
                totalPrice = priceFor15Days + additionalPrice;
...
//     }
    ...
    $('#form_checkout').on('submit', function (e) {
        ...
        let inputCarBrand = $('input#checkout_car_model').val()
        if (inputCarBrand == '') {
